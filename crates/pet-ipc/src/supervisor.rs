...
pub struct Host {
    pub avatar_capabilities: pet_protocol::AvatarCapabilities,
    child: Child,
...
impl Host {
    pub fn start(command: &mut Command, timeout: Duration) -> Result<Self> {
        ...
        let mut host = Self {
            avatar_capabilities: Default::default(),
            child,
            requests: Some(tx),
            responses: reply_rx,
            worker: Some(worker),
        ...
        let ready = host.exchange("hello", "ready", json!({}))?;
        ensure!(
        ...
        host.avatar_capabilities =
            serde_json::from_value(ready["payload"]["avatar"].clone()).unwrap_or_default();
        Ok(host)
    }
    ...
    fn exchange(&mut self, kind: &str, expected: &str, payload: Value) -> Result<Value> {
    ...
    pub fn desktop(&mut self, command: pet_protocol::DesktopCommand) -> Result<()> {
        let reply = self.exchange("desktop", "result", serde_json::to_value(command)?)?;
            ...
            reply["payload"]["accepted"].as_bool() == Some(true),
    ...
    pub fn poll(&mut self) -> Result<Vec<pet_protocol::AvatarEvent>> {
        let reply = self.exchange("poll", "events", json!({}))?;
        let events: Vec<pet_protocol::AvatarEvent> =
            serde_json::from_value(reply["payload"]["events"].clone())?;
        ensure!(events.len() <= 32, "event batch too large");
        Ok(events)
    }
    pub fn avatar(&mut self, command: pet_protocol::AvatarCommand) -> Result<()> {
        let reply = self.exchange("avatar", "result", serde_json::to_value(command)?)?;
        ensure!(
            reply["payload"]["accepted"] == true,
            "avatar command rejected: {}",
            reply["payload"]["error"]
        );
        Ok(())
    }
    ...
    pub fn ping(&mut self) -> Result<()> {
        self.exchange("ping", "pong", json!({})).map(|_| ())
    }

    pub fn shutdown(mut self) -> Result<()> {
        self.exchange("shutdown", "stopped", json!({}))?;
        self.requests.take();
...
/// Finite health-check helper for lifecycle tests. Each restart creates a fresh protocol session.
pub fn supervise(
