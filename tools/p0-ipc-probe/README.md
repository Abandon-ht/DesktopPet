...
边界：目前只监督直接子进程；不允许宿主把控制管道继承给其后代，否则线程清理可能等待未关闭的管道。还没有进程树管理、GUI 可取消的持续监控任务、Tauri 生命周期接入或真实 winit 窗口关闭验证。子试件本身仍依赖 EOF 退出，没有独立心跳看门狗。下一步把控制消息投递到渲染主线程，并接入 Tauri 后验证实际应用关闭、崩溃恢复与句柄继承；P0-04 整体仍未通过。
