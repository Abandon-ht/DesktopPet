...
以 1 名熟悉 Rust 与语音推理的开发者投入为估算，不包含额外美术制作、授权获取和模型训练时间。人日表示有效工作量而非承诺日期；阶段允许根据 P0 测量调整。P0 Gate 已通过，实际结果见 [P0 验证记录](07-p0-validation.md)；P1 本地 alpha 已完成，P2–P7 尚未实施。
...
当前状态（2026-09-24）：**P1-01 至 P1-06 本地 alpha 完成，用户要求进入下一阶段。**100 次交互、跨屏、Space、睡眠恢复及 Finder/Safari/编辑器吸附均获用户确认。最终跟随包的通知到角色移动 p95 约 4.5 ms，用户确认比前版更顺畅、仍有少量延迟，未报告新异常；该延迟被接受，不再为此增加本阶段资源开销。present 调用间隔 p95 约 35.0 ms，未达到原 33.3 ms 目标；能耗未直接测量，且不同功能曾用独立测试包验证。这些偏差保留为后续发行前的观察项，证据见 [P1-06 结果](14-p1-06-alpha-validation.md)。以下阶段整体目标保持不变。
