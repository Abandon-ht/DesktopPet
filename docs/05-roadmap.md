...
以 1 名熟悉 Rust 与语音推理的开发者投入为估算，不包含额外美术制作、授权获取和模型训练时间。人日表示有效工作量而非承诺日期；阶段允许根据 P0 测量调整。P0 已开始，实际结果见 [P0 验证记录](07-p0-validation.md)；P1–P7 尚未实施。
...
当前状态：P0-01 release 构建通过；P0-02 已有参数/表情检查和渲染图；P0-03 已通过本机人工验收（采用用户确认的焦点和命中余量规则）；P0-05/06 仅取得部分证据，P0-04 已开始，独立管道协议、EOF 退出及 Supervisor 超时/有限重启试件通过，Tauri/渲染宿主接入和真实窗口恢复待完成。**P0 Gate 尚未通过**，任务通过条件保持如下，不以部分试件替代完整验收。
